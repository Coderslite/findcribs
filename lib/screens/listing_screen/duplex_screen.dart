...
class DuplexScreen extends StatefulWidget {
  ...
  const DuplexScreen({Key? key, required this.duplexType}) : super(key: key);
...
class _DuplexScreenState extends State<DuplexScreen> {
  ...
  int currentDuplexIndex = 0;
  int currentStateIndex = 0;
  int currentPropertyTypeIndex = 0;

  handleGetProperties() {
    propertyList = getPropertyListCategory(widget.duplexType, page);
  ...
  int minPrice = 0;
  int maxPrice = 0;
  FilterDuplexController filterDuplexController =
      Get.put(FilterDuplexController());

  handleFilter() {
    print("filter");
    print(filterDuplexController.propertyType);
    setState(() {
      isLoading = true;

      propertyList = filterPropertyCategory(
          filterDuplexController.propertyType.string == 'All'
              ? ""
              : filterDuplexController.propertyType.string,
          ...
          filterDuplexController.state.toString() == 'Nigeria'
              ? ''
              : filterDuplexController.state.toString(),page);
      propertyList.then((value) {
        filteredList = [];
    ...
    propertyList = filterPropertyCategory(
        ...
        filterDuplexController.state.toString() == 'Nigeria'
            ? ''
            : filterDuplexController.state.toString(),
        page);
    propertyList.then((value) {
      // print(value);
      if (value.isEmpty) {
                            ...
                            GestureDetector(
                              ...
                              child: Container(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    Text(
                                      filterDuplexController.propertyType
                                          .toString(),
                                      style: const TextStyle(
                                          fontFamily: 'RedHatDisplay',
                                          fontWeight: FontWeight.w700,
