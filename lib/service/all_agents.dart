...
class AllAgentController extends GetxController {
  ...
  Future<void> fetchAgents(int pageKey) async {
        ...
        if (posts.isNotEmpty) {
          if (pageKey == 1 && agentPagingController.itemList != null) {
            ...
            for (var post in posts.toList()) {
              // Using toList() to create a new list to avoid concurrent modification issues

              if (_posts.any((element) => element.userId == post.userId)) {
                posts.remove(post);
                print("removed");
                update();
              }
